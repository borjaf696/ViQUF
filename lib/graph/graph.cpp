            ...
            &basic_information, &true_nodes, &traslation_map](OwnNode_t src, OwnNode_t target, 
                ...
                */
                ...
                if (STRATEGY == 3)
                {
                    ...
                    if (Parameters::get().debug && neigh_assigned != INF)
...
void DBG::export_graphs_sc()
{
    ...
    {
        int cur_node = nodes_to_traverse.top();
        nodes_to_traverse.pop();
        if (traversed_nodes[cur_node])
            continue;
        valid_nodes.emplace(cur_node);
        min_node_active = (cur_node < min_node_active)?cur_node:min_node_active;
...
void DBG::_set_edge_frequency(OwnNode_t node, OwnNode_t neighbor, float new_flow, bool additive)
{
            ...
            if (!skip) {
                ...
                if ((out_flow > (1 + FLOW_LIMIT_RATIO) * in_flow) || (out_flow * (1 + FLOW_LIMIT_RATIO) < in_flow)) {
                    ...
                    for (size_t j = 0; j < min_flows_unary.size(); ++j) {
                        ...
                        if (show)
                            adjust_information << " Val ori: " << _g_freqs[n._id][neighs_to_process[j]] << " Val trad: " << min_flows_unary[j]<< endl;
                    }
                    if (show)
...
priority_queue<pair<size_t,vector<OwnNode_t>>> DBG::get_min_cost_flow_paths(float pre_flow)
{
    ...
    switch (status) {
        ...
        case NS::OPTIMAL:
        {
            ...
            for (Graph::ArcIt a(fn); a != INVALID; ++a){
                available_flow[a] = capacity[a] - ns.flow(a);
...
priority_queue<pair<size_t,vector<OwnNode_t>>> DBG::solve_std_mcp(const vector<string> & sequence_map, bool flag_full_unitigs_map)
{
    ...
    for (auto start_point: sources_nodes)
    {
        OwnNode_t id = start_point._id;
        if (Parameters::get().t_data == "TGS")
            if (!_g_nodes[id]._active || isolated(_g_nodes[id]._id))
                continue;
    ...
    for (size_t i = 0; i < _g_edges.size(); ++i)
    {
        ...
        {    
            if (Parameters::get().t_data == "TGS")
                if (!_g_nodes[_g_edges[i][j]]._active || isolated(_g_nodes[_g_edges[i][j]]._id))
                    continue;
            if (Parameters::get().debug)
    ...
    switch (status) {
        ...
        case NS::OPTIMAL:
        {
            ...
            while(true)
            {
                ...
                if (flow == 0 || flow == -INF) {
                    if (Parameters::get().debug)
                        nw_file << "No more paths available!"<<endl;
                    break;
                }
                if (Parameters::get().debug)
                    nw_file << "And the winner is... " << flow << " From: " << translation_map[fn.source(selected)]<<" "<< translation_map[fn.target(selected)]<< endl;
                cur_path.push_back(selected);
                __flow_to_path(fn.target(selected), cur_path, flow);
                if (Parameters::get().debug)
                ...
                if (Parameters::get().t_data == "TGS")
                {
                    if (flow > 1)
                    {
                        ...
                        for (size_t i = 0; i < real_path.size(); ++i)
                            _set_edge_frequency(real_path[i], real_path[i+1], flow, true);
                    }
                }
